...
async function loadRecentActivity() {
  try {
    ...
    // Map activity types to readable verbs
    const typeLabel = (type) => {
      switch ((type || '').toLowerCase()) {
        case 'issue': return 'Borrowed';
        case 'return': return 'Returned';
        case 'add_member': return 'Member Joined';
        case 'add_book': return 'Book Added';
        default: return type || 'Activity';
      }
    };

    container.innerHTML = activities.map(a => {
      // Prefer joined fields, fall back to message
      const bookTitle = a.book_title || '';
      const member = a.member_name || '';
      const verb = typeLabel(a.type);

      // Format date safely
      let dateStr = '';
      if (a.created_at) {
        const d = new Date(a.created_at);
        dateStr = (isNaN(d.getTime())) ? (a.created_at || '') : d.toLocaleString();
      }

      // Title line: show book title when present, otherwise a shortened message
      const titleLine = bookTitle || (a.message || '').slice(0, 80) || verb;

      // Status class: returned -> returned, issue -> active
      const statusClass = (a.type === 'return') ? 'returned' : (a.type === 'issue' ? 'active' : 'info');
